...
from quantum.islands.core.kernel import LivniumQubit, normalize
from quantum.islands.utils.bloch_to_cube import bloch_to_cube, cube_to_bloch
...
class GeometricQuantumSimulator:
    ...
    def apply_cnot_between_positions(self, control_pos: Tuple[int, int, int],
        ...
        from quantum.islands.core.kernel import EntangledPair
...
def create_large_geometric_system(n_qubits: int, 
                                   grid_size: Optional[int] = None,
                                   qubits_per_position: int = 1) -> GeometricQuantumSimulator:
    """
    Create a large-scale geometric quantum system with n_qubits.
    
    This function can create systems with 5000+ qubits by using larger grid sizes.
    
    Args:
        n_qubits: Target number of qubits
        grid_size: Optional grid size. If None, computed automatically to fit n_qubits
        qubits_per_position: Number of qubits per cube position (layering, default 1)
        
    Returns:
        GeometricQuantumSimulator with n_qubits qubits
        
    Example:
        # Create 5000-qubit system
        simulator = create_large_geometric_system(5000)
        print(f"Created: {len(simulator.all_qubits)} qubits")
        print(f"Memory: {simulator.get_memory_usage()}")
    """
    # Auto-compute grid_size if not provided
    if grid_size is None:
        # Compute minimum grid_size needed
        positions_needed = (n_qubits + qubits_per_position - 1) // qubits_per_position
        grid_size = int(np.ceil(positions_needed ** (1/3)))
        # Round up to next integer
        if grid_size ** 3 < positions_needed:
            grid_size += 1
    
    simulator = GeometricQuantumSimulator(grid_size=grid_size)
    
    qubit_count = 0
    
    # Fill grid with qubits
    for x in range(grid_size):
        for y in range(grid_size):
            for z in range(grid_size):
                ...
                # Add qubits at this position
                for layer in range(qubits_per_position):
                    if qubit_count >= n_qubits:
                        break
                    
                    value = np.random.rand()
                    if qubits_per_position > 1:
                        qubit_id = f"q_{x}_{y}_{z}_L{layer}"
                    else:
                        qubit_id = f"q_{x}_{y}_{z}"
                    simulator.add_qubit((x, y, z), value, qubit_id)
                    qubit_count += 1
